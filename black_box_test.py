...
def main():
    ...
    parser.add_argument('--sub_model', default='smallCNN',
                    type=str, required=False, help='substitude model type')
    parser.add_argument('--sub_model_path', default='./saved_model/sub_smallcnn/model.pt',
                        type=str, help='substitude model path')                    
    ...
    sub_model = get_model(args.sub_model)
    sub_model = load_model(sub_model, args.sub_model_path)
    ...
    for img, label in tqdm(test_loader):
        ...
        if args.attack == 'fgsm':
            ...
            perturbed_img = pgd_attack(img, label, device, sub_model, step_size=args.step_size, step_num=args.step_num, epsilon=args.epsilon)   
        elif args.attack == 'mi_fgsm':
            perturbed_img = mi_fgsm_attack(img, label, device, sub_model, step_size=args.step_size, step_num=args.step_num, epsilon=args.epsilon, alpha=args.alpha)     
        else:
